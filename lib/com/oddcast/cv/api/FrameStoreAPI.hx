...
import com.oddcast.cv.util.JPEGEncodeOddcast;
	...
	class FrameStoreAPI extends FaceTracker 
	{
		
		public function new(trackerMode:TrackerMode) {
			#if debugtrace
				com.oddcast.util.Utils.debugTrace("new(trackerMode:"+trackerMode+")");
			#end
			super(trackerMode);
		...
		#if testing
		override public function setTraceFilter(filter:com.oddcast.util.trace.TraceFilterWhiteAndBlackLists) {
			super.setTraceFilter(filter);
			if (filter!=null) {
				//filter.addAllFilterToWhiteList("FrameStore Archive TextureWriter");
			}
		}
		#end
		...
		public function createFrameStore_FB(maxFrames:Int):FrameStoreID {
			#if debugtrace
				com.oddcast.util.Utils.debugTrace("createFrameStore_FB(maxFrames:"+maxFrames+")");
			#end
			trace("createFrameStore_FB");
			var frameStore = new FrameStore(imageProvider, maxFrames);
		...
		public function getNumberOfFrames(id:FrameStoreID):Int {
			#if debugtrace
				com.oddcast.util.Utils.debugTrace("getNumberOfFrames(frameStoreID:"+id+")");
			#end
			return frameStore(id).getNumberOfFrames();
		}
