 ...
 **/
	public class BlitMask extends Sprite {
		...
		/** @private  **/
		protected function _render(xOffset:Number=0, yOffset:Number=0, clear:Boolean=true, limitRecursion:Boolean=false, xNudge:Number=0, yNudge:Number=0):void {
			...
			//nudge the values to avoid slight vibrations during animation. Adjust for these in the Matrix that gets passed to beginBitmapFill()
			if (clear) {
				xNudge = 1 - (_sliceRect.x % 1);
				if (xNudge > 0.5) {
					xNudge -= 1;
				}
				yNudge = 1 - (_sliceRect.y % 1);
				if (yNudge > 0.5) {
					yNudge -= 1;
				}
			}
			
			_sliceRect.x += xNudge - 1; //subtract 1 to make sure the whole image gets included - without this, a very slight vibration can occur on the edge during animation.
			_sliceRect.y += yNudge - 1;
			...
			if (_wrap && clear) {
				...
				_render(Math.ceil((_bounds.x - x) / wrapWidth) * wrapWidth, Math.ceil((_bounds.y - y) / wrapHeight) * wrapHeight, false, false, xNudge, yNudge);
			} else if (_smoothing) {
			...
			if (_wrap) {
				...
				if (x + _width > _bounds.right) {
					_render(xOffset - wrapWidth, yOffset, false, true, xNudge, yNudge);
				} 
				if (!limitRecursion && y + _height > _bounds.bottom) {
					_render(xOffset, yOffset - wrapHeight, false, false, xNudge, yNudge);
				}
			}
			
			if (clear) {
				_tempMatrix.tx = xNudge - 1; //subtract 1 to compensate for the pixel we added above where we created the nudge values.
