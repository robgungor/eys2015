		...
		***************************** interface methods */
		public function open_win(_action:String=null):void {	
			...
			if (_action == "fromComeback") {
				...
				image_positioner.setAnchor(0, 35, true);  //sets the zoom/rotate focus point on the red dotted line of the elf's mouth /Jake
				...
				if (face_location != null) {	
					...
					curRot =  -face_location[3];
					curScale = face_location[2] * 0.75;      //scales the photo to more or less fit the elf cutout.
					
					
					var shiftDown:Point = new Point(0.0, 57.0 );  //add on a translation to bring the head down so the photo mouth matches the elf's red dotted line mouth
					//This translation is going to be put onto the bmp which is already transformed by the rotation and scale, 
					//so we have to compensate for that here by mutliplying in the inverse transform.
					// It's still not working fully, but is more or less right for vagualy upright photos. /Jake
					
					var matrix:Matrix = new Matrix();
					matrix.rotate(curRot);
					matrix.scale(curScale,curScale);
					matrix.invert();
					var tShiftDown = matrix.transformPoint(shiftDown);
					curX = -face_location[0] + tShiftDown.x;
					curY = -face_location[1] + tShiftDown.y;
					
					curRot *=  180  / Math.PI; //convert to degrees
				}else {
